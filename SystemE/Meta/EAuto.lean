...
def collectAllLemmas'
  (axioms : Array Name) (hintstx : TSyntax ``hints) (uords : Array (TSyntax `Auto.uord)) (ngoalAndBinders : Array FVarId) :
  -- The first `Array Lemma` are `Prop` lemmas
  -- The second `Array Lemma` are Inhabitation facts
  TacticM (Array Lemma × Array Lemma) := do
  let mut inputHints ← parseHints hintstx
  for x in axioms do
...
| `(eauto | eauto $instr $hints $[$uords]*) => withMainContext do
  ...
  withMainContext do
    ...
    match instr with
    | .none =>
      let axioms := (← getEuclidTheorems).post.values.map (·.origin.key) |>.toList.dedup.toArray
